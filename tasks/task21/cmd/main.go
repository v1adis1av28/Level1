...
func (c *Car) FillCar() {
	fmt.Println("Fill car with fuel type: ", c.FuelType)
}
...
func (a *Adapter) LoadCar() {
	a.car.FillCar()
}

func main() {
	kamaz := Car{FuelType: "DT"}
	jeep := Car{FuelType: "100"}

	adapterKamaz := Adapter{car: &kamaz}
	adapterJeep := Adapter{car: &jeep}

	adapterJeep.LoadCar()
	adapterKamaz.LoadCar()
}
